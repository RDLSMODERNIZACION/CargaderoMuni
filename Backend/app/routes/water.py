...
# =========================
@router.get("/admin/cleanup-old-storage-photos")
async def cleanup_old_storage_photos(confirm: str = ""):
    keep_path = "photos/dispatch_1/prueba_162.jpg.png"

    if confirm != "keep-162-delete-old":
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        if listed.status_code not in (200, 201):
            raise HTTPException(
                status_code=502,
                detail={"stage": "list", "status": listed.status_code, "body": listed.text},
            )

        # Storage list at a prefix is not recursive, so use DB metadata through
        # the Storage API's generic remove endpoint by known paths supplied
        # from storage.objects via a backend query.
        ...
        for object_path in paths:
            delete_url = f"{SUPABASE_URL}/storage/v1/object/{STORAGE_BUCKET}/{object_path}"
